	    ...
	    <div class="input-field">
		    ...
		    <option value="" disabled selected>ESCOLHA UM TIPO</option>	
		      @foreach($tipo_itens as $tipo_item)                    
                   <option value="{{ $tipo_item->id }}" @if (isset($registro->id_tipo)) @if ($registro->id_tipo == $tipo_item->id) selected  @endif @endif > {{ $tipo_item->nome }} </option> 
              @endforeach
		    </select>
		...
		<BR>
		 ...
		 <div class="input-field">
			<select multiple name="categorias[]" id="categorias">
			    @if (empty($categoria_itens)) 
			    	<option value="" disabled selected>SELECIONE UM TIPO DE OBJETO PRIMEIRO</option>	
			    @else
				    @foreach($categoria_itens as $categoria_item)                    
	                   <option value="{{ $categoria_item->id }}" @if(preg_match(",$categoria_item->id,", $registro->categorias)) selected  @endif  > {{ $categoria_item->nome  }} </option> 
	                @endforeach
                @endif
			</select>
